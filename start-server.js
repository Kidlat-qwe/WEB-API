    ...
    <body>
      ...
      <script>
            ...
            tableBody.innerHTML = students.map(student => 
              '<tr>' +
                ...
                '<td class="action-buttons">' +
                  ...
                  '<button class="delete-button" onclick="deleteStudent(' + student.student_id + ')">Delete</button>' +
                '</td>' +
              '</tr>'
            ).join('');
          } catch (error) {
        ...
        function addStudent() {
          ...
          alert('Add student functionality will be implemented');
        }

        function editStudent(id) {
          // Implement edit student functionality
          alert('Edit student functionality will be implemented for ID: ' + id);
        }
...
// Add teachers page route separately
app.get('/teachers', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html>
    <head>
      <title>Teachers - School Management Database</title>
      <style>
        * {
          margin: 0;
          padding: 0;
    ...
    <body>
      ...
      <div class="container">
        <div class="header">
          <h1 class="page-title">Teachers Data</h1>
          <button class="add-button" onclick="addTeacher()">Add Teacher</button>
        </div>
      ...
      <script>
          ...
          try {
            ...
            tableBody.innerHTML = teachers.map(teacher => 
              '<tr>' +
                ...
                '<td><span class="status-' + (teacher.status === 'ACTIVE' ? 'active' : 'inactive') + '">' + 
                  (teacher.status || 'N/A') + '</span></td>' +
                '<td class="action-buttons">' +
                  '<button class="edit-button" onclick="editTeacher(\'' + teacher.teacher_id + '\')">Edit</button>' +
                  '<button class="delete-button" onclick="deleteTeacher(\'' + teacher.teacher_id + '\')">Delete</button>' +
                '</td>' +
              '</tr>'
            ).join('');
          } catch (error) {
            ...
            document.getElementById('teachersTableBody').innerHTML = 
              '<tr><td colspan="7" style="text-align: center; color: #dc2626;">Failed to load teachers data. Please try again later.</td></tr>';
          }
        }

        function addTeacher() {
          // Implement add teacher functionality
          alert('Add teacher functionality will be implemented');
        }

        function editTeacher(id) {
          // Implement edit teacher functionality
          alert('Edit teacher functionality will be implemented for ID: ' + id);
        }
        ...
        window.addEventListener('load', fetchTeachers);
      </script>
    </body>
    </html>
  `);
